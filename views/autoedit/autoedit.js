  ...
  function AAutomaton(a, noSave) {
        ...
        apiPost("createAutomaton", { "name": self.Name, "type": "DEA" })
          .success(function(data) {
            ...
            if (data.result == "OK") {
              ...
              apiPost("saveAutomaton", {
                "id": data.automaton.ID, "name": data.automaton.Name,
          ...
          if (n == alphabets[1].join("")) self.selectedAlphabet = "A1"; else
            if (n == alphabets[2].join("")) self.selectedAlphabet = "A2"; else
              if (n == alphabets[3].join("")) self.selectedAlphabet = "A3"; else
                if (!$('#userAlphabetInput').is(":focus")) { self.selectedAlphabet = "A4"; self.userAlphabetInput = self.automaton.Alphabet.join(","); }
      }
          ...
          if (n == stackAlphabets[1].join("")) self.selectedStackAlphabet = "A1"; else
            if (n == stackAlphabets[2].join("")) self.selectedStackAlphabet = "A2"; else
              if (n == stackAlphabets[3].join("")) self.selectedStackAlphabet = "A3"; else
                if (!$('#userStackAlphabetInput').is(":focus")) { self.selectedStackAlphabet = "A4"; self.userStackAlphabetInput = self.automaton.StackAlphabet.join(","); }
      }
              ...
              for (var z = m.state.Transitions[t].Labels.length - 1; z >= 0; z--) { // will use epsilon last
                if (m.state.Transitions[t].Labels[z] == c || m.state.Transitions[t].Labels[z] == '') { // epsilon case
                  ...
                  if (clone || self.simulationTrace[i].accept) {
                    ...
                    self.simulationTrace.push({
                      index: self.simulationTrace.length,
                      items: cloneTraceNEA(self.simulationTrace[i].items, m.step + 1, m.state.Transitions[t], z)
                    }); // add new machine
              ...
              for (var z = 0; z < m.state.Transitions[t].Labels.length; z++) {
                if (m.state.Transitions[t].Labels[z][0] == c &&
                  ...
                  if (clone || self.simulationTrace[i].accept) {
                    ...
                    self.simulationTrace.push({
                      index: self.simulationTrace.length,
                      items: cloneTraceNKA(self.simulationTrace[i].items, m.step + 1, m.state.Transitions[t], z)
                    }); // add new machine
          ...
          for (var i = 0; i < state.Transitions.length; i++) {
            for (var z = 0; z < state.Transitions[i].Labels.length; z++) {
              ...
              if (self.symbolMatches(readSymbol, c, self.automaton.blankSymbol)) {
                ...
                } else if (writeSymbol.includes('*')) {
                  if (writeSymbol.startsWith('*') && writeSymbol.endsWith('*')) {
                    //  *middle* -> just write what was read
                    stack[stackPointer] = c;
                  } else if (writeSymbol.startsWith('*')) {
                    //  *suffix -> extract prefix from read and add suffix
                    var suffix = writeSymbol.substring(1);
                    if (readSymbol.endsWith('*')) {
                      var readPrefix = readSymbol.substring(0, readSymbol.length - 1);
                      if (c.startsWith(readPrefix)) {
                        stack[stackPointer] = c.substring(0, c.length - suffix.length) + suffix;
                      } else {
                        stack[stackPointer] = c; // fallback
                      }
                    } else {
                      stack[stackPointer] = c; // write what was read
                    }
                  } else if (writeSymbol.endsWith('*')) {
                    // prefix* -> extract suffix from read and add prefix
                    var prefix = writeSymbol.substring(0, writeSymbol.length - 1);
                    if (readSymbol.startsWith('*')) {
                      var readSuffix = readSymbol.substring(1);
                      if (c.endsWith(readSuffix)) {
                        stack[stackPointer] = prefix + c.substring(prefix.length);
                      } else {
                        stack[stackPointer] = c; // fallback
                      }
                    } else {
                    ...
                    stack[stackPointer] = writeSymbol; // literal write
                  }
                } else {
    ...
    self.moveDrag = function(e) {
      ...
      if (self.newTransition) {
        ...
        self.newTransition.pathPoints[1] = {
          x: self.newTransition.Source.x + self.newTransition.x,
    ...
    reader.onload = function(data) {
      try {
        ...
        if (data.target.result.indexOf('<?xml version="1.0"') === 0) {
          ...
          a = {
            name: filename, description: "", type: "",
